...
class MigrationGenerator extends BaseCommand
{
    ...
    protected function prepare(string $class): string
    {
        ...
        if ($this->getOption('session')) {
            ...
            $data['DBDriver'] = Axm::config('Database')->{$data['DBGroup']}['DBDriver'];
            $data['matchIP']  = Axm::config('App')->sessionMatchIP;
        }
    ...
    {
        return gmdate(Axm::config('Migrations')->timestampFormat) . basename($filename);
    }
}
