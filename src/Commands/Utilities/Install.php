...
class Install extends BaseCommand
{
    ...
    public function run(array $params)
    {
        ...
        if (!is_file($composerJsonPath)) {
            CLI::error('No se encontró el archivo composer.json en el directorio actual.');
            CLI::newLine();
            return;
        }
    ...
    private function runCommand(string $command)
    {
        try {
            exec($command);
            CLI::success('El paquete se instaló correctamente.');
            CLI::newLine();
        } catch (Exception $e) {
            CLI::error('Ocurrió un error al instalar el paquete:');
