...
class CLI
{
    ...
    public static function prompt(string $field, $options = null, $validation = null): string
    {
        ...
        if ($validation && !is_array($validation) && !is_string($validation)) {
            throw new AxmException(Axm::t('axm', '$rules can only be of type string|array'));
        }
    ...
    public static function promptByKey($text, array $options, $validation = null): string
    {
        if (is_string($text)) {
            ...
            throw new AxmException(Axm::t('axm', '$text can only be of type string|array'));
        }

        if (!$options) {
            throw new AxmException(Axm::t('axm', 'No options to select from were provided'));
        }
    ...
    public static function color(string $text, string $foreground, ?string $background = null, ?string $format = null): string
    {
        ...
        if (!array_key_exists($foreground, static::$foreground_colors)) {
            throw new AxmException(Axm::t('axm', ' foreground Invalid Color'));
        }

        if ($background !== null && !array_key_exists($background, static::$background_colors)) {
            throw new AxmException(Axm::t('axm', ' background Invalid Color'));
        }
     ...
     * @return mixed
     */
    public static function getOption(string $name, string $default = null)
    {
