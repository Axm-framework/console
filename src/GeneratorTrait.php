...
trait GeneratorTrait
{
    ...
    protected function execute(array $params): void
    {
        ...
        if ($this->getOption('namespace') === 'Axm') {
            ...
            CLI::write('CLI generator using Axm Namespace: ', 'yellow');
            CLI::newLine();

            if (CLI::prompt('Are you sure you want to continue?', ['y', 'n'], 'required') === 'n') {
                CLI::newLine();
                CLI::write('CLI generator cancel Operation ', 'yellow');
                CLI::newLine();
        ...
        if (!$this->getOption('force') && $isFile) {
            CLI::error('CLI generator file Exist: ' . realpath($path), 'light_gray', 'red');
            CLI::newLine();
        ...
        if (!write_file($path, $this->buildContent($class))) {
            // @codeCoverageIgnoreStart
            CLI::error('CLI generator file Error: ' . realpath($path), 'light_gray', 'red');
            CLI::newLine();
        ...
        if ($this->getOption('force') && $isFile) {
            CLI::write('CLI generator file Over write: ' . realpath($path), 'yellow');
            CLI::newLine();

            return;
        }

        CLI::write('CLI generator file Create: ' . realpath($path), 'green');
        CLI::newLine();
    }
    ...
    protected function qualifyClassName(): string
    {
        ...
        $class = CLI::getSegment(2); // OJO esta linea originalmente era asi  $this->params[0] ?? CLI::getSegment(2);

        if ($class === null && $this->hasClassName) {
            // @codeCoverageIgnoreStart
            $nameLang = $this->classNameLang ?: 'Console.generator.className.default';
            $class    = CLI::prompt(lang($nameLang), null, 'required');
            CLI::newLine();
            // @codeCoverageIgnoreEnd
        }

        helpers('inflector', true);
    ...
    {
        $templatePath = __DIR__ . "/Commands/Generators/Views/{$this->template}";
        $output = View::captureFile($templatePath, $data);
    ...
    protected function buildPath(string $class): string
    {
        ...
        $base = [$namespace];
        if (!$base = reset($base)) {
            CLI::error('CLI namespace Not Defined: ' . [$namespace], 'light_gray', 'red');
            CLI::newLine();
